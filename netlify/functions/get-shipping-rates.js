...
<body>
    <div class="top-bar">
        <div class="top-bar-content">
            ...
            <div>
                ...
                <a href="cart.html">🛒 Cart <span id="cart-count" class="cart-badge" style="display: none;">0</span></a>
            </div>
        </div>
    </div>
        ...
        <div class="product-layout">
            ...
            <div class="product-info">
                ...
                <div class="price-section">
                    ...
                    <div class="shipping">Calculate shipping below</div>
                </div>
                <div class="option-section">
                    <div class="option-label">📍 Item Location:</div>
                ...
                <div class="option-section" style="background: #f9f9f9; padding: 20px; border-radius: 8px;">
                    <div class="option-label">📦 Calculate Shipping Cost</div>
                ...
                <div class="action-buttons">
                    ...
                    <button class="btn btn-secondary" onclick="buyNow()">Buy It Now</button>
                    <button class="btn btn-secondary wishlist-btn" id="watchlist-btn" onclick="toggleWatchlist()">
                        <span id="heart-icon">♡</span> <span id="watchlist-text">Add to Watchlist</span>
                    </button>
                </div>
                ...
                <div class="seller-section">
                    <div class="seller-header">
                        ...
                        <div class="seller-rating">⭐ 98.5% positive</div>
                    </div>
    ...
    <script>
        // ✅ PRODUCT DATA - Properly defined with sellerAddress
        const product = { 
            id: 'asics-matflex-6', 
            name: 'ASICS Matflex 6 Wrestling Shoes - Black/Silver', 
            price: 49.99, 
            image: '👟', 
            condition: 'Used - Excellent', 
            seller: 'wrestlingpro23', 
            sellerId: 'user123',
            sellerAddress: {
                name: 'wrestlingpro23',
                city: 'Vestal',
        ...
        // DEBUG: Log product on page load
        console.log('✅ Product loaded:', product);
        console.log('✅ Seller address:', product.sellerAddress);
        ...
        function checkWatchlistStatus() { const watchlist = getWatchlist(); const isInWatchlist = watchlist.some(item => item.id === product.id); if (isInWatchlist) { document.getElementById('watchlist-btn').classList.add('in-watchlist'); document.getElementById('heart-icon').textContent = '♥'; document.getElementById('watchlist-text').textContent = 'In Watchlist'; } }
        function showToast(message, type = 'success') { const existingToast = document.querySelector('.toast'); if (existingToast) { existingToast.remove(); } const toast = document.createElement('div'); toast.className = `toast ${type}`; toast.innerHTML = `<span style="font-size: 20px;">${type === 'success' ? '✓' : '✕'}</span><span>${message}</span>`; document.body.appendChild(toast); setTimeout(() => { toast.classList.add('hiding'); setTimeout(() => toast.remove(), 300); }, 3000); }
        ...
        function toggleWatchlist() { const watchlist = getWatchlist(); const existingIndex = watchlist.findIndex(item => item.id === product.id); if (existingIndex > -1) { watchlist.splice(existingIndex, 1); document.getElementById('watchlist-btn').classList.remove('in-watchlist'); document.getElementById('heart-icon').textContent = '♡'; document.getElementById('watchlist-text').textContent = 'Add to Watchlist'; showToast('Removed from watchlist'); } else { watchlist.push({ ...product, addedAt: new Date().toISOString() }); document.getElementById('watchlist-btn').classList.add('in-watchlist'); document.getElementById('heart-icon').textContent = '♥'; document.getElementById('watchlist-text').textContent = 'In Watchlist'; showToast('Added to watchlist!'); } saveWatchlist(watchlist); }
        ...
        async function calculateShipping() {
            ...
            console.log('🚀 Sending to API:', {
                zipCode: zipCode,
                senderAddress: product.sellerAddress
            });
            ...
            try {
                ...
                if (rates && rates.length > 0) {
                    ...
                    resultsDiv.innerHTML = '<div class="shipping-error">No shipping rates available</div>';
                }
        ...
        function displayShippingRates(rates) {
            ...
            let html = '<div style="font-size: 13px; font-weight: bold; margin-bottom: 10px; color: #333;">Shipping from ' + product.sellerAddress.city + ', ' + product.sellerAddress.state + ':</div>';
            
            sortedRates.forEach((rate, index) => {
                const price = parseFloat(rate.amount);
                const days = rate.estimated_days || rate.duration_terms || 'N/A';
                const carrier = rate.provider || 'Carrier';
                const service = rate.servicelevel?.name || rate.servicelevel_name || 'Standard';
                
                html += `
                    <div class="shipping-rate" onclick="selectShippingRate('${rate.object_id || index}', ${price})">
        ...
        function selectShippingRate(rateId, price) { 
            selectedShippingRate = { id: rateId, price: price }; 
            document.querySelectorAll('.shipping-rate').forEach(el => { el.classList.remove('selected'); }); 
            event.currentTarget.classList.add('selected'); 
            updatePriceWithShipping(price); 
            showToast(`Shipping option selected: $${price.toFixed(2)}`); 
        }
